...
| `--metrics-exporter` | Expose the IORails non-streaming admission-queue metrics (`guardrails.nonstream.*`) on a scrape endpoint. Supported value: `prometheus`. Overrides `NEMO_GUARDRAILS_SERVER_METRICS_EXPORTER`. Not exported by default. |
