...
class PIIRecognizer:
    def __init__(
        self, config: RailsConfig, load_predefined: bool = False, redact: bool = False
    ):
        ...
        if config.sensitive_data_detection:
            ...
            if not config.sensitive_data_detection.provider:
                ...
                if config.sensitive_data_detection.entities:
                    self._map_pii_entities_from_config(
            ...
            for name, obj in inspect.getmembers(module):
                if inspect.isclass(obj) and hasattr(obj, "recognizer_meta"):
                    try:
                        ...
                        log.info(
                            "failed to add recognizer to registry of analyzer engine"
                        )
